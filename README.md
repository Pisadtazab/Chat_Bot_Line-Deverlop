...
│   ├── services/
│   │   └── line_bot.py                  # LINE webhook processing and delivery
...
│       ├── extractPDF.py                # upload, extract, summarize, and ingest PDFs
...
# หรือผ่าน Docker (เลือก tunnel เพียงตัวเดียว)
...
`POST /upload_pdf` keeps the original RAG ingestion flow:

1. Save the uploaded PDF under `src/`.
2. Extract each page's text and images with PyMuPDF.
3. Create a Thai summary with `StelleX/mt5-base-thaisum-text-summarization`.
...
```json
{
  "message": "research.pdf uploaded and processed",
  "summary": "Generated Thai summary"
}
```
